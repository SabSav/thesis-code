...
def main(args):
    ...
    for _ in tqdm.tqdm(range(args.burn_in), desc="Burn-in Alg1"): chain.advance()

    for i in tqdm.tqdm(range(args.length[0]), desc="Simulation T0 Alg1"):
        chain.advance()
...
def simulate(
    ...
    # Skip the first burn_in samples so that the stationary distribution is reached
    for _ in tqdm.tqdm(range(burn_in), desc="Burn-in Alg1"): chain.advance()

    for i in tqdm.tqdm(range(length[0]), desc="Simulation T0 Alg1"):
        chain.advance()
